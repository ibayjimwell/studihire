...
import { Plus, Trash2, Loader2, ImageIcon, Link2, X, Pencil } from 'lucide-react';

const EMPTY_ITEM = { title: '', description: '', image_url: '', project_url: '', category: '', tags: [] };
...
export default function StudentPortfolioTab({ profileId }) {
  ...
  useEffect(() => {
    base44.entities.Portfolio.filter({ student_profile_id: profileId }, '-created_date', 50)
      .then(data => { setItems(data); setLoading(false); });
  }, [profileId]);

  const openNew = () => { setEditing(null); setForm(EMPTY_ITEM); setTagInput(''); setDialogOpen(true); };
  const openEdit = (item) => { setEditing(item); setForm({ ...item }); setTagInput(''); setDialogOpen(true); };
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      ...
      setItems(is => is.map(i => i.id === editing.id ? { ...i, ...data } : i));
    } else {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" className="gradient-primary text-white border-0 gap-1" onClick={openNew}>
      ...
      {items.length === 0 && (
        <Card className="border-dashed border-border">
          <CardContent className="flex flex-col items-center py-12 text-center">
            ...
            <p className="font-medium text-foreground text-sm">No portfolio items yet</p>
            <p className="text-xs text-muted-foreground mt-1">Showcase your best work to attract clients.</p>
            <Button size="sm" className="mt-4 gradient-primary text-white border-0" onClick={openNew}>Add First Item</Button>
          </CardContent>
        </Card>
      )}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            <div>
              <Label>Title *</Label>
              <Input className="mt-1" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. E-commerce Website Redesign" />
            </div>
            <div>
              <Label>Description</Label>
              <Textarea className="mt-1" rows={3} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Describe this project..." />
            </div>
            <div>
              ...
              <label className="mt-1 flex flex-col items-center justify-center p-4 border-2 border-dashed border-border rounded-lg cursor-pointer hover:border-primary hover:bg-primary/5 transition-all">
                {form.image_url
                  ? <img src={form.image_url} className="w-full h-28 object-cover rounded" alt="cover" />
                  : imageUploading
                    ? <Loader2 className="w-6 h-6 animate-spin text-primary" />
                    : <><ImageIcon className="w-6 h-6 text-muted-foreground mb-1" /><span className="text-xs text-muted-foreground">Click to upload image</span></>
                }
            ...
            <div>
              ...
              <Input className="mt-1" value={form.project_url} onChange={e => setForm(f => ({ ...f, project_url: e.target.value }))} placeholder="https://..." />
            </div>
            <div>
              <Label>Category</Label>
              <Input className="mt-1" value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} placeholder="e.g. Web Design, Mobile App" />
            </div>
            <div>
              <Label>Tags</Label>
          ...
          <DialogFooter>
            ...
            <Button className="gradient-primary text-white border-0" onClick={handleSave} disabled={saving || !form.title}>
