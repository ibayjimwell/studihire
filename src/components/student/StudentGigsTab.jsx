...
import { Briefcase, Plus, Star, Pencil, Trash2, Loader2, Eye, EyeOff } from 'lucide-react';
...
export default function StudentGigsTab({ studentId, verificationStatus }) {
  ...
  useEffect(() => {
    base44.entities.Gig.filter({ student_id: studentId }, '-created_date', 20)
  ...
  const toggleStatus = async (gig) => {
    ...
    setGigs(gs => gs.map(g => g.id === gig.id ? { ...g, status: newStatus } : g));
    setTogglingId(null);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {verificationStatus === 'approved' && (
          <Button size="sm" className="gradient-primary text-white border-0 gap-1" asChild>
            <Link to="/student/gigs/new"><Plus className="w-4 h-4" /> New Gig</Link>
          </Button>
        )}
      </div>
      ...
      {verificationStatus !== 'approved' && gigs.length === 0 && (
        <Card className="border-dashed border-border">
          <CardContent className="flex flex-col items-center py-12 text-center">
            ...
            <p className="font-medium text-foreground text-sm">Verification required</p>
            <p className="text-xs text-muted-foreground mt-1">You can post gigs once your profile is approved by an admin.</p>
          </CardContent>
        </Card>
      )}
      ...
      {gigs.length === 0 && verificationStatus === 'approved' && (
        <Card className="border-dashed border-border">
          <CardContent className="flex flex-col items-center py-12 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Start offering your services!</p>
            <Button size="sm" className="mt-4 gradient-primary text-white border-0" asChild>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {gigs.map(gig => (
          <Card key={gig.id} className="border-border overflow-hidden">
            {gig.cover_image_url && (
              <img src={gig.cover_image_url} alt={gig.title} className="w-full h-32 object-cover" />
            )}
            <CardContent className="p-4 space-y-2">
              <div className="flex items-start justify-between gap-2">
                <p className="text-sm font-semibold text-foreground leading-tight">{gig.title}</p>
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ${STATUS_COLORS[gig.status]}`}>
                  {gig.status}
                </span>
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <Button size="sm" variant="outline" className="flex-1 h-7 text-xs gap-1"
                  onClick={() => toggleStatus(gig)} disabled={togglingId === gig.id}>
                  {togglingId === gig.id
                ...
                <Button size="sm" variant="outline" className="h-7 w-7 p-0 text-destructive hover:bg-destructive/10"
                  onClick={() => deleteGig(gig.id)} disabled={deletingId === gig.id}>
                  {deletingId === gig.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />}
                </Button>
              </div>
            </CardContent>
          </Card>
