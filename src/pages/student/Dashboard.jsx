...
import {
  GraduationCap, Briefcase, MessageSquare, DollarSign,
  Star, Plus, TrendingUp, Clock, CheckCircle, LayoutDashboard,
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      base44.entities.Payment.filter({ student_id: user?.id }, '-created_date', 5),
    ]).then(([profiles, g, p]) => {
  ...
  const totalEarnings = payments.filter(p => p.status === 'released').reduce((s, p) => s + (p.net_amount || 0), 0);
  const pendingEarnings = payments.filter(p => p.status === 'pending').reduce((s, p) => s + (p.net_amount || 0), 0);

  const stats = [
    { label: 'Total Earned', value: `₱${totalEarnings.toLocaleString()}`, icon: DollarSign, color: 'text-green-600', bg: 'bg-green-50' },
    { label: 'Pending', value: `₱${pendingEarnings.toLocaleString()}`, icon: Clock, color: 'text-yellow-600', bg: 'bg-yellow-50' },
    { label: 'Active Gigs', value: gigs.filter(g => g.status === 'active').length, icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Rating', value: profile?.rating ? `${profile.rating.toFixed(1)}★` : '—', icon: Star, color: 'text-purple-600', bg: 'bg-purple-50' },
  ];
      ...
      {profile?.verification_status === 'pending' && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-yellow-700 text-xs mt-0.5">Our team is reviewing your school ID. You'll be able to post gigs once approved.</p>
          </div>
        </div>
      )}
      {profile?.verification_status === 'rejected' && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-red-700 text-xs mt-0.5">{profile.verification_notes || 'Please re-upload your school ID.'}</p>
            <Button size="sm" variant="outline" className="mt-2 border-red-300 text-red-700" asChild>
            ...
            {gigs.length === 0 ? (
              <div className="text-center py-8">
                ...
                {profile?.verification_status === 'approved' && (
                  <Button size="sm" className="mt-3 gradient-primary text-white border-0" asChild>
              ...
              payments.map(p => (
                <div key={p.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-bold text-primary">₱{p.net_amount?.toLocaleString()}</p>
                    <span className={`text-xs ${p.status === 'released' ? 'text-green-600' : 'text-yellow-600'}`}>
                      {p.status}
                    </span>
                  </div>
                </div>
