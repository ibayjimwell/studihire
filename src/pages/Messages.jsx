...
export default function Messages() {
  ...
  useEffect(() => {
    if (!user) return;
    base44.entities.Conversation.list('-last_message_at', 50).then(setConversations);
  }, [user]);

  useEffect(() => {
    if (!activeConv) return;
    base44.entities.Message.filter({ conversation_id: activeConv.id }, 'created_date', 100).then(setMessages);
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto">
        <div className="flex h-screen pt-0" style={{ height: 'calc(100vh - 0px)' }}>
          {/* Conversation list */}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-semibold text-foreground truncate">{otherName}</span>
            ...
            {!activeConv ? (
              ...
              <>
                ...
                <div className="p-4 bg-white border-b border-border flex items-center gap-3 shadow-sm">
                  ...
                  <div>
                    <p className="font-semibold text-foreground text-sm">{getOtherName(activeConv)}</p>
                ...
                <div className="p-4 bg-white border-t border-border">
                  <div className="flex gap-2">
                    <Input
                      ...
                      className="flex-1"
                    />
                    <Button onClick={sendMessage} disabled={sending || !newMsg.trim()} className="gradient-primary text-white border-0">
